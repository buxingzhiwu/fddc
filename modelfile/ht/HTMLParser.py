...
class HTMLParser(object):
    ...
    def parse_content_hetong_v2(self, html_file_path, flag=False):
        rs = []
        with codecs.open(html_file_path, encoding='utf-8', mode='r') as fp:
            soup = BeautifulSoup(fp.read(), "html.parser")
            [s.extract() for s in soup('table')]
        ...
        if flag:
            for i in range(len(article)):
                article[i] = TextUtils.del_space(article[i])
        return article
...
if __name__ == "__main__":
    Parser = HTMLParser()
    DICT = Parser.parse_content_new2("../../data/复赛新增类型训练数据-20180712/资产重组/html/1095063.html")#tc:懒得改了
